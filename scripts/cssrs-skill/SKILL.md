...
### 3. 管理后台 `/admin`
管理员手机浏览器访问，输入管理密码 `8888` → 创建/撤销医生准入码、查看所有患者量表。
- **医生管理**：创建/撤销医生 PIN
...
```bash
curl -s -H "X-Doctor-PIN: <PIN>" http://<SERVER>:8888/api/doctor/report
```
...
```bash
curl -s -H "X-Admin-PIN: <管理密码>" http://<SERVER>:8888/api/report
```
...
**查看所有 PIN：**
```bash
curl -s -H "X-Admin-PIN: <管理密码>" http://<SERVER>:8888/api/admin/pins
```
